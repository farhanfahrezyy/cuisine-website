...
<style>
  .uk-nav-sub {
    ...
    /* Saat menu terbuka */
    .uk-open > .uk-nav-sub {
        max-height: 1000px; /* Nilai tinggi maksimum yang cukup besar */
        opacity: 1;
    }
    ...
    .uk-nav-parent-icon > li.uk-open > a::after {
        content: '−';
        transform: rotate(90deg); /* Ubah rotasi menjadi 90 derajat */
    }
</style>
@endpush
        ...
        <div data-uk-grid>
            <div class="uk-width-1-4@m sticky-container">
                <div data-uk-sticky="offset: 100; bottom: true; media: @m;">
                    <h2>Recipes</h2>
                    <ul class="uk-nav-default uk-nav-parent-icon uk-nav-filter uk-margin-medium-top"
                    data-uk-nav>
    <li class="uk-parent">
                        ...
                        <div
                            ...
                            <div class="p-4 md:p-5">
                                ...
                                <div class="flex justify-between items-center mb-3">
                                    ...
                                    <span
                                        class="font-semibold
                                            ...
                                            @else
                                                {{ $recipe->price == 0 ? 'text-green-600' : 'text-black-600' }}
                                            @endif
                                                ">
                                        @if (Auth::check() && Auth::user()->role === 'user')
                                ...
                                <div class="flex flex-wrap sm:flex-nowrap justify-between items-center gap-2">
                                    ...
                                    @if (Auth::check() && Auth::user()->role === 'user')
                                    <a href="{{ route('recipes.show', ['id' => $recipe->id]) }}" class="uk-text-primary font-s, mediwhitespace-nowrap">
                                        Lihat Resep →
                                    </a>
                                    @elseif (!Auth::check())
                                    <a href="{{ route('recipes.show', ['id' => $recipe->id]) }}" class="uk-text-primary font-s, mediwhitespace-nowrap">
                                    ...
                                    @else
                                    <a href="{{ route('user.login.form') }}" class="uk-text-primary font-s, mediwhitespace-nowrap">
                                        Lihat Resep →
                                    </a>
                                    @endif
                                </div>
    ...
    parents.forEach(parent => {
        ...
        link.addEventListener('click', function(e) {
            ...
            if (parent.classList.contains('uk-open')) {
                submenu.style.maxHeight = submenu.scrollHeight + 'px'; // Atur tinggi berdasarkan konten
            } else {
